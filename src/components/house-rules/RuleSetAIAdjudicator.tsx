...
interface RuleSetAIAdjudicatorProps {
  ruleSetName: string;
  gameName: string;
  rules: HouseRule[];
  voice?: string;
...
export const RuleSetAIAdjudicator = ({ 
  ruleSetName,
  gameName,
  rules,
  voice = "alloy"
}: RuleSetAIAdjudicatorProps) => {
  const houseRules = rules.map(r => r.rule_text);
  ...
  const contextPrompt = `I'm playing ${gameName} with the "${ruleSetName}" house rules. `;
  ...
  const handleSend = async (messageOverride?: string, shouldSpeak?: boolean) => {
    ...
    try {
      ...
      console.error("[RuleSetAIAdjudicator] Error in sendMessage:", error);
  ...
  const speakResponse = async (text: string) => {
    try {
      ...
      console.error("[RuleSetAIAdjudicator] TTS error:", error);
  ...
  const startRecording = async () => {
    try {
      ...
      console.error("[RuleSetAIAdjudicator] Error accessing microphone:", error);
  ...
  const transcribeAudio = async (audioBlob: Blob) => {
    try {
      ...
      console.error("[RuleSetAIAdjudicator] Transcription error:", error);
  ...
  const startRealtimeChat = async () => {
    try {
      ...
      const rulesContext = rules.map((r, i) => `Rule ${i + 1}: ${r.rule_text}`).join("\n");
      
      const contextInstructions = `You are a helpful card game rules expert for ${gameName}. 
The player is using the "${ruleSetName}" house rules set.
...
Keep responses under 3 sentences unless more detail is requested.`;
      ...
      console.error("[RuleSetAIAdjudicator] Error starting realtime chat:", error);
  ...
  return (
    <section className="animate-slide-up" aria-label="AI Adjudicator">
        ...
        <div className="p-6">
          ...
          <div className="flex flex-col items-center py-6 mb-6">
            ...
            <p className="mt-4 text-sm text-muted-foreground text-center">
              Press to speak with your House Rules AI.
            </p>
          </div>
